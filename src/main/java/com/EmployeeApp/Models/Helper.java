...
public class Helper {
    ...
    public List<Employee> findEmployeesByLanguagesAndScore(String language, int minimumScore) {
        ...
        // Sort result list by score in ascending order
        Collections.sort(result, new Comparator<Employee>() {
            @Override
            public int compare(Employee e1, Employee e2) {
                // Assuming KnownLanguage has a method getScoreOutof100() to get the score
                int score1 = getScoreForLanguage(e1, language);
                int score2 = getScoreForLanguage(e2, language);

                // Sort in ascending order
                return Integer.compare(score1, score2);  // Ascending order
            }

            private int getScoreForLanguage(Employee employee, String language) {
                for (KnownLanguage emlanguage : employee.getKnownLanguages()) {
                    if (emlanguage.getLanguageName().equals(language)) {
                        return emlanguage.getScoreOutof100();
                    }
                }
                return 0; // Default if the language is not found
            }
        });
